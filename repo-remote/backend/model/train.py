...
from backend.model.lstm_model import build_model  # noqa: E402
...
# New feature columns: norm_speed, avg_congestion_index, hour_sin, hour_cos, eta_seconds, weather_temp, weather_humidity, weather_rain
FEATURE_COLUMNS = [
    "norm_speed",
...
def train_for_location(location: str) -> dict:
    """Train and evaluate LSTM model for one location's preprocessed arrays with extended features."""
    ...
    x = np.load(x_path)
    y = np.load(y_path)
    # If legacy shape, skip location
    if x.shape[-1] < len(FEATURE_COLUMNS):
        print(f"[WARN] Skipping {location}: not enough features in X (expected {len(FEATURE_COLUMNS)}, got {x.shape[-1]})")
        return {"location": location, "error": "not enough features"}
    if len(x) < 5:
    ...
    model = build_model(input_shape=(3, len(FEATURE_COLUMNS)))
