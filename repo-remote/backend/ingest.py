...
# Ensure .env is loaded for ingestion
from dotenv import load_dotenv
import os
load_dotenv(dotenv_path=os.path.join(os.path.dirname(__file__), ".env"), override=True)
print("Ingestion using database:", os.getenv("DATABASE_URL"))
...
from backend.database import CORRIDORS, RawTraffic, SessionLocal, init_db
...
def fetch_weather(lat, lng):
    ...
    try:
        ...
        print(f"[WEATHER][WARN] {lat},{lng}: {e}")
        return {}
...
INGEST_BATCH_MULTIPLIER = max(1, int(os.getenv("INGEST_BATCH_MULTIPLIER", "12")))
...
CORRIDOR_PROBES = {
    ...
    "Koramangala": [(12.9352, 77.6245), (12.9328, 77.6269), (12.9384, 77.6213)],
    "Church Street": [(12.9755, 77.6058), (12.9738, 77.6071), (12.9772, 77.6041)],
    "JP Nagar": [(12.9077, 77.5850), (12.9049, 77.5876), (12.9106, 77.5824)],
    "Jayanagar": [(12.9250, 77.5938), (12.9219, 77.5924), (12.9284, 77.5956)],
    "Commercial Street": [(12.9826, 77.6088), (12.9841, 77.6112), (12.9807, 77.6067)],
    "KR Market": [(12.9633, 77.5760), (12.9614, 77.5791), (12.9652, 77.5734)],
}
...
def _fallback_row(session, location_name: str, lat: float, lng: float) -> dict:
    ...
    if latest is not None:
        ...
        crowd_density = min(1.0, max(0.0, 0.3 + 0.7 * congestion_index))
        return {
...
def _pull_location(location_name: str, lat: float, lng: float) -> dict:
    """Fetch corridor using multiple probes and return hotspot-like aggregate, with ETA and weather."""
    ...
    # Calculate ETA (current_travel_time in seconds)
    eta_seconds = hotspot["current_travel_time"]
    # Fetch weather
    ...
    return {
        ...
        "crowd_density": hotspot.get("crowd_density", 0.0),
...
def run_loop() -> None:
    ...
    while True:
        try:
            ...
            # Trigger preprocessing after each batch
            try:
                ...
                print(f"[PREPROCESS][TRIGGERED] {datetime.utcnow().isoformat()}Z\n" + result.stdout)
            except subprocess.CalledProcessError as pexc:
                print(f"[PREPROCESS][ERROR] {pexc}\n{pexc.stderr}")
        except Exception as exc:
